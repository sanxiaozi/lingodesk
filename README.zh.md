...
- **稳健**:三级引擎链 —— Claude 主引擎 + OpenAI / DeepSeek 自动兜底,熔断器秒级切换、恢复自动切回;`business_connection_id` 实时刷新并持久化;每客户语种锁定;7 天无往来自动归档、来新消息自动复活
...
- **免费版**(云托管)—— 每月 300 条翻译回复,无限联系人、全部功能。手机上完成 Telegram 侧设置,把 bot token 私聊发给 [@LingoDeskbot](https://t.me/LingoDeskbot) 即**秒级开通**(翻译引擎我们提供)。见[图文教程](https://lingodesk.org/zh/setup/),约 10 分钟。
- **Pro** —— **500 ⭐/月(≈¥50)**,Telegram Stars 支付:无限翻译 + 优先支持,随时取消。
- **自托管** —— **永久免费**(MIT),快速开始见下。同一套代码天然多租户:你的实例也能托管别人(他们把 token 私聊发给*你的*门户 bot),且计费默认关闭。
...
5. 私聊你自己的 bot,把 `.env` 里那个 token 发给它 —— 持有 token 即所有权证明,立即把你注册为第一个租户。控制台群是可选的:不建群,客户消息直接出现在你和 bot 的私聊里(对卡片「回复」即可作答);客户多了想分线管理,再建话题群发 `/bind`。群 ID 和用户 ID 全自动识别,不用查任何数字。
