...
/** 下载文件到 storage/<contactId>/<fileName>,返回本地路径或 null */
export async function downloadTgFile(fileId: string, contactId: string, fileName: string): Promise<string | null> {
  try {
    const gf = await fetch(`https://api.telegram.org/bot${config.botToken}/getFile?file_id=${fileId}`);
    ...
    const url = `https://api.telegram.org/file/bot${config.botToken}/${gj.result.file_path}`;
    const buf = Buffer.from(await (await fetch(url)).arrayBuffer());
    const dir = join(config.storageDir, contactId);
